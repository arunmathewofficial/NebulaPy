...
def compute_luminosity(workerQ, doneQ):
    ...
    while True:
        try:
            ...
            if recombination:
                print(f" multiprocessing: computing the recombination line(s) luminosity of {ion_name:<4} lines")
                ...
                print(f" multiprocessing: computing the collisional de-excited line(s) luminosity of {ion_name:<4} lines")
            ...
            doneQ.put({ion_name: luminosity, 'recombination': recombination, 'mask': ism_mask})  # Store result
...
if __name__ == "__main__":
    ...
    # todo: require two files one for grid mask and another for shocked ism mask
    outfile = os.path.join(output_dir, filename)
    ...
    with open(outfile, "w") as file:
        ...
        file.write("#Task: Multiprocessing for Computing the Temporal Evolution of Hα, Hβ Luminosities\n\n")
    ...
    for step, silo_instant in enumerate(batched_silos):
        ...
        all_processes = {
            ...
            "HalphaCollGrid": (line_emission, Chianti_HAlphaLines, H_num_density, False, grid_mask, False),
            "HalphaRecoGrid": (line_emission, PyNeb_HAlphaLines, H1P_num_density, True, grid_mask, False),
            ...
            "HalphaCollISM": (line_emission, Chianti_HAlphaLines, H_num_density, False, shocked_ism_mask, True),
            "HalphaRecoISM": (line_emission, PyNeb_HAlphaLines, H1P_num_density, True, shocked_ism_mask, True),
            "HbetaCollISM": (line_emission, Chianti_HBetaLines, H_num_density, False, shocked_ism_mask, True),
            "HbetaRecoISM": (line_emission, PyNeb_HBetaLines, H1P_num_density, True, shocked_ism_mask, True),
        }

        # get keys of ions
        keys = all_processes.keys()
        computed_result = {}
