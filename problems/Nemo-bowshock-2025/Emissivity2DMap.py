...
output_dir = '/home/tony/Desktop/multi-ion-bowshock/sim-output/emissiviity_map'  # Output image directory
# Set the directory containing the silo files and the base name of these files
silo_dir = '/home/tony/Desktop/multi-ion-bowshock/sim-output/silo'  # Directory with simulation data (silo files)
...
pion_ion = 'He1+'  # Ion of interest (Oxygen IV)
lines = [303.78, 303.786]  # Wavelengths of the emission lines in Angstroms
...
batched_silos = util.batch_silos(
    ...
    start_time=start_time,  # Optional start time
    finish_time=finish_time,  # Optional finish time
    time_unit='kyr',  # Optional time units
    out_frequency=out_frequency  # Optional output frequency
)
...
N_grid_level = geometry['Nlevel']  # Number of grid levels
...
for step, silo_instant in enumerate(batched_silos):
    ...
    emissivity_map_dict = line_emission.line_emissivity_map_cylindrical(lines, temperature, ne, progress_bar=True)

    h5_filename = f"{filebase}_emiss_{ion_name}_{sim_time.value:.4e}kyr.h5"
    ...
    for line in emissivity_map_dict:
        ...
        filename = f"{filebase}_emiss_{line_name}_{sim_time.value:.4e}kyr.png"
        filepath = os.path.join(ion_output_dir, filename)
        plt.savefig(filepath, bbox_inches="tight", dpi=300)
        plt.close(fig)

        print(f" saving emissivity map ion: {pion_ion}, line: {line}, to {filename}")
